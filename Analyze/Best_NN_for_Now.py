...
def predictions_after_nn(desired_week, desired_year, df, file_path):
    ...
    for v in range(0, 100, 1):
        ...
        y_train = pd.Series()
        check_year = desired_year - 2
        current_week_games = df[df['Year'] < check_year]
        ...
        # Weekly loop
        for year in range(check_year, df['Year'].max() + 1, 1):
            for week in range(df[df['Year'] == year]['week_number'].min(),
                              df[df['Year'] == year]['week_number'].max() + 1,
                              1):
